...
public class ISOStringFieldPackager extends FieldPackager
{
    ...
    public byte[] pack(IField<?> c) throws MessageException
    {
        try
        {
            ...
            byte[] rawData = interpreter.interpret(paddedData);
            
            byte[] d = new byte[prefixer.getPackedLength() + rawData.length];
            
            prefixer.encodeLength(paddedData.length(), d);
            System.arraycopy(rawData, 0, d, prefixer.getPackedLength(), rawData.length);
            return d;
        } catch(Exception e)
        {
    ...
    public int unpack(IField<?> c, byte[] b, int offset) throws MessageException
    {
        try
        {
            ...
            ((IField<String>)c).setValue(unpacked);
