...
public class AsciiPrefixer implements Prefixer
{
	 ...
	 */
    public void encodeLength(int length, byte[] b) throws MessageException
    {
    	this.encodeLength(length, b, 0);
    }
    
    public void encodeLength(int length, byte[] b, int offset) throws MessageException
    {
    	  int n = length;
          ...
          {
              b[i+offset] = (byte)(n % 10 + '0');
              n /= 10;
          }
          if (n != 0)
          {
              throw new MessageException("invalid len "+ length + ". Prefixing digits = " + nDigits);
          }
    }
